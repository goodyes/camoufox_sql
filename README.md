# camoufox_sql
All Camoufox inside a T-SQL (MS SQL) database. Ready to use in your software. For data only. Comes empty.

Based on Camoufox v.135.0.1-beta.24 from March 15, 2025 which as of 2026 March 04 is still the last working v.

It ports all the Camoufox properties (fingerprints) into a SQL Database. 

Run schema.sql into your database, just change "USE 'YOUR_DATABASE_NAME'" for the name of your database.


![camoufoxsql](https://github.com/user-attachments/assets/055c140d-93fe-4e64-9d2b-96948321c615)
